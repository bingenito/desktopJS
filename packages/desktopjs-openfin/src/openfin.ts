...
export class OpenFinContainer extends WebContainerBase {
    ...
    // Default name generated by OpenFin for windows defined in manifest snapshot
    private static readonly INTERNAL_GENERATED_WINDOW = 'internal-generated-window';
    ...
    protected closeAllWindows(): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            ...
            this.desktop.Application.getCurrent().getChildWindows(windows => {
                const { name: mainWindow } = this.getMainWindow();

                windows.forEach(window => {
                    if (!window.name.includes(mainWindow)) {
    ...
    private getManifest(): Promise<any> {
        return new Promise((resolve, reject) => {
            this.desktop.Application.getCurrent().getManifest(manifest => {
                resolve(manifest);
            }, reject);
        })
    }
